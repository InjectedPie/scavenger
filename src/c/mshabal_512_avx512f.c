...
void simd512_mshabal_avx512f(mshabal512_context *sc, const void *data0, const void *data1, const void *data2,
    ...
    num = len >> 6;
    if (num != 0) {
        mshabal_compress_avx512f(sc, (const unsigned char *)data0, (const unsigned char *)data1,
    ...
    memcpy(sc->buf0, data8, len);
    memcpy(sc->buf1, data9, len);
    memcpy(sc->buf2, data10, len);
    memcpy(sc->buf3, data11, len);
    memcpy(sc->buf4, data12, len);
    memcpy(sc->buf5, data13, len);
    memcpy(sc->buf6, data14, len);
    memcpy(sc->buf7, data15, len);
    sc->ptr = len;
}

void mshabal256_close_avx512f(mshabal512_context *sc, unsigned ub0, unsigned ub1, unsigned ub2,
